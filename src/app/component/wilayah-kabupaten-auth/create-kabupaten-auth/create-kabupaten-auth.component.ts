...
@Component({
  selector: 'app-create-kabupaten-auth',
  templateUrl: './create-kabupaten-auth.component.html',
  styleUrls: ['./create-kabupaten-auth.component.css'],
})
export class CreateKabupatenAuthComponent implements OnInit {
  constructor(
    private title: Title,
  ...
  ngOnInit(): void {
    this.title.setTitle('Buat Kabupaten Auth');
    ...
    this.services
      ...
      .subscribe((result) => {
        ...
        if (result.body.status === true) {
          ...
          this.router.navigate(['/create-kabupaten']);
        } else {
          this.toggleLoading.showLoading(false);
          this.router.navigate(['/unauthorized']);
        }
