...
@Component({
  selector: 'app-edit-kelurahan-auth',
  templateUrl: './edit-kelurahan-auth.component.html',
  styleUrls: ['./edit-kelurahan-auth.component.css'],
})
export class EditKelurahanAuthComponent implements OnInit {
  constructor(
    private title: Title,
  ...
  ngOnInit(): void {
    this.title.setTitle('Edit Kelurahan Auth');
    ...
    this.services
      ...
      .subscribe((result) => {
        ...
        if (result.body.status === true) {
          ...
          this.router.navigate(['/edit-kelurahan/' + this.idParameter]);
        } else {
          this.toggleLoading.showLoading(false);
          this.router.navigate(['/unauthorized']);
        }
