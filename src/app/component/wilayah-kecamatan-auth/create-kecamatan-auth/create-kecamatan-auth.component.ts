...
@Component({
  selector: 'app-create-kecamatan-auth',
  templateUrl: './create-kecamatan-auth.component.html',
  styleUrls: ['./create-kecamatan-auth.component.css'],
})
export class CreateKecamatanAuthComponent implements OnInit {
  constructor(
    private title: Title,
  ...
  ngOnInit(): void {
    this.title.setTitle('Buat Kecamatan Auth');
    ...
    this.services
      ...
      .subscribe((result) => {
        ...
        if (result.body.status === true) {
          ...
          this.router.navigate(['/create-kecamatan']);
        } else {
          this.toggleLoading.showLoading(false);
          this.router.navigate(['/unauthorized']);
        }
