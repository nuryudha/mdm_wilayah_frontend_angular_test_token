...
@Component({
  selector: 'app-edit-kecamatan-auth',
  templateUrl: './edit-kecamatan-auth.component.html',
  styleUrls: ['./edit-kecamatan-auth.component.css'],
})
export class EditKecamatanAuthComponent implements OnInit {
  constructor(
    private title: Title,
  ...
  ngOnInit(): void {
    this.title.setTitle('Edit Kecamatan Auth');
    ...
    this.services
      ...
      .subscribe((result) => {
        ...
        if (result.body.status === true) {
          ...
          this.router.navigate(['/edit-kecamatan/' + this.idParameter]);
        } else {
          this.toggleLoading.showLoading(false);
          this.router.navigate(['/unauthorized']);
        }
