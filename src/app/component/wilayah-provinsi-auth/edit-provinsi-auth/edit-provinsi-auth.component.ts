...
@Component({
  selector: 'app-edit-provinsi-auth',
  templateUrl: './edit-provinsi-auth.component.html',
  styleUrls: ['./edit-provinsi-auth.component.css'],
})
export class EditProvinsiAuthComponent implements OnInit {
  constructor(
    private title: Title,
  ...
  ngOnInit(): void {
    this.title.setTitle('Edit Provinsi Auth');
    ...
    this.services
      ...
      .subscribe((result) => {
        ...
        if (result.body.status === true) {
          ...
          this.router.navigate(['/edit-provinsi/' + this.idParameter]);
        } else {
          this.toggleLoading.showLoading(false);
          this.router.navigate(['/unauthorized']);
        }
