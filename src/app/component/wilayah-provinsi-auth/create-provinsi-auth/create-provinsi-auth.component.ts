...
@Component({
  selector: 'app-create-provinsi-auth',
  templateUrl: './create-provinsi-auth.component.html',
  styleUrls: ['./create-provinsi-auth.component.css'],
})
export class CreateProvinsiAuthComponent implements OnInit {
  constructor(
    private title: Title,
  ...
  ngOnInit(): void {
    this.title.setTitle('Buat Provinsi Auth');
    ...
    this.services
      ...
      .subscribe((result) => {
        ...
        if (result.body.status === true) {
          ...
          this.router.navigate(['/create-provinsi']);
        } else {
          this.toggleLoading.showLoading(false);
          this.router.navigate(['/unauthorized']);
        }
